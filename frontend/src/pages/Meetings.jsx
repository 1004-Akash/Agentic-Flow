...
const Meetings = ({ triggerMeetingDemo, loading: globalLoading }) => {
  ...
  const [participants, setParticipants] = useState(["Alex Chen (You)", "Marcus V.", "Elena G."]);
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; ++i) {
          if (event.results[i].isFinal) {
            ...
            const segment = { user: "Alex Chen", text: final, timestamp: new Date().toLocaleTimeString() };
  ...
  const joinMeeting = () => {
    const id = "MT-" + Math.floor(Math.random() * 1000000);
    setMeetingId(id);
    setIsInMeeting(true);
    
    // Connect to WebSocket (mocked connection but logic is here)
    socketRef.current = new WebSocket(`ws://localhost:8000/ws/${id}/Alex Chen`);
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-12 custom-scroll flex flex-col items-center justify-center bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#1A2235] to-[#111623]">
        <div className="w-full max-w-2xl bg-[#151B2B] border border-[#2D3748] rounded-[40px] p-12 shadow-[0_40px_100px_rgba(0,0,0,0.6)] text-center relative overflow-hidden">
            ...
            >
                ...
                <span>Initialize Secure Meeting Node</span>
            </button>
    ...
    <div className="flex-1 flex flex-col h-full bg-[#0B0F19]">
            ...
            <div className="flex items-center gap-4 border-l border-[#2D3748] pl-6 ml-2">
                <div className="flex -space-x-2">
                    {participants.map((p, i) => (
                        <div key={i} className="w-8 h-8 rounded-full bg-slate-700 border-2 border-[#111623] flex items-center justify-center text-[10px] font-bold text-white uppercase">{p.substring(0,2)}</div>
                    ))}
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 p-8 grid grid-cols-2 gap-8 overflow-y-auto custom-scroll">
                {participants.map((p, i) => (
                    <div key={i} className={`bg-[#151B2B] rounded-[32px] border-2 transition-all relative flex flex-col items-center justify-center overflow-hidden ${(p.includes('Alex') && isListening) ? 'border-indigo-500 shadow-[0_0_30px_rgba(79,70,229,0.2)]' : 'border-[#2D3748]'}`}>
                        <div className="flex flex-col items-center relative z-10">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                <div className={`w-2 h-2 rounded-full ${isListening && p.includes('Alex') ? 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,1)] animate-pulse' : 'bg-slate-600'}`}></div>
                                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">{isListening && p.includes('Alex') ? 'Transcribing Live' : 'Nominal'}</span>
                            </div>
                        </div>
                        {/* Audio Wavefront Mockup for active speaker */}
                        {isListening && p.includes('Alex') && (
                ...
                <div className="bg-[#151B2B]/40 rounded-[32px] border-2 border-dashed border-[#2D3748] flex items-center justify-center flex-col gap-4 group cursor-pointer hover:bg-[#151B2B]/60 transition-all">
            ...
            <div className="w-[380px] border-l border-[#2D3748] flex flex-col bg-[#111623]">
                ...
                <div className="p-6 border-t border-[#2D3748] bg-[#0B0F19]">
                    ...
                    <div className="relative group">
                        <button className="w-full h-12 bg-[#1A2235] border border-[#2D3748] hover:border-indigo-500/50 rounded-xl px-4 flex items-center justify-between transition-all">
