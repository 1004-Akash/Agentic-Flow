...
import { 
  CheckSquare, Clock, Filter, Search, ChevronRight, User, AlertCircle, CheckCircle
} from 'lucide-react';
...
const Tasks = ({ tasks }) => {
  return (
    <div className="flex-1 overflow-y-auto px-8 pb-12 custom-scroll flex flex-col">
      ...
      <div className="mb-6 flex items-center gap-4 sticky top-0 bg-[#111623] py-4 z-10 shrink-0">
      ...
      <div className="space-y-4">
          {tasks.length > 0 ? tasks.map((t, i) => (
            <div key={t._id || i} className="bg-[#1A2235] border border-[#2D3748] rounded-2xl p-6 hover:border-indigo-500/50 transition-all flex items-center gap-8 group">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border ${t.needs_clarification ? 'bg-[#2d2215] text-amber-500 border-amber-500/30' : 'bg-[#052e16] text-emerald-500 border-emerald-500/30'}`}>
                            {t.needs_clarification ? 'Awaiting Logic' : 'Nominal'}
                        </span>
                    </div>
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-2 text-slate-500">
                            ...
                            <span className="text-[12px] font-medium leading-none">SLA: 02:45h remaining</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex -space-x-2">
                        {[1,2,3].map(v => (
                            <div key={v} className="w-8 h-8 rounded-full bg-[#252E46] border-2 border-[#1A2235] flex items-center justify-center text-[10px] font-bold text-slate-400">A{v}</div>
                        ))}
                    </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-[#1A2235] border border-dashed border-[#2D3748] rounded-3xl">
                <CheckSquare className="w-16 h-16 text-slate-700 mb-4 opacity-30" />
                <p className="text-slate-500 font-bold uppercase tracking-widest text-[12px]">No Predictive Vectors Found</p>
                <p className="text-slate-600 text-[13px] mt-1">Ingest a meeting transcript to generate autonomous tasks.</p>
            </div>
          )}
      </div>

      <div className="mt-12 p-8 border border-indigo-500/20 bg-indigo-500/5 rounded-3xl relative overflow-hidden shrink-0">
          <div className="relative z-10 flex items-center justify-between">
              <div className="max-w-xl">
                  <h3 className="text-xl font-bold text-white mb-2 leading-tight">Autonomous Re-allocation Engine</h3>
                  <p className="text-[14px] text-slate-400 leading-relaxed font-medium">The FixAgent is currently monitoring workload distributions. If any owner becomes over-encumbered, the system will re-route tasks automatically.</p>
              </div>
              <button className="px-8 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[14px] rounded-xl transition-all shadow-[0_0_20px_rgba(99,102,241,0.3)]">Force Rebalance</button>
          </div>
          <Zap className="absolute right-[-20px] top-[-20px] w-48 h-48 text-indigo-500/10 rotate-12" />
      </div>
    </div>
  );
};
